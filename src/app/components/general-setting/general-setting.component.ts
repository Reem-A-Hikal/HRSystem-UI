...
export class GeneralSettingComponent implements OnInit {
  ...
  additional: number = 0;
  deduction: number = 0;
  paymentType: SettingType = SettingType.Hour;
  holiday1: string = 'Friday';
  holiday2: string = 'Saturday';
...
get additionalInvalid(): boolean {
  if (this.paymentType === SettingType.Hour) {
    const val = Number(this.additional);
    return isNaN(val) || val < 1 || val > 10 || val * 2 !== Math.floor(val * 2); // allow .5 steps
  } else {
    const val = Number(this.additional);
